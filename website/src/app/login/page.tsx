  ...
  return (
    <div className="w-full max-w-md bg-[#111111] border border-[#333333] rounded-3xl p-8 md:p-12 shadow-2xl relative overflow-hidden">
      ...
      <div className="space-y-4 relative z-10">
        <button className="w-full bg-white text-black hover:bg-gray-200 py-3 rounded-xl font-bold transition-colors flex items-center justify-center gap-3">
          <Github className="w-5 h-5" />
          {isSignup ? 'Sign up with GitHub' : 'Continue with GitHub'}
        </button>
        
        <button className="w-full bg-[#1A1A1A] border border-[#333333] hover:border-gray-500 text-white py-3 rounded-xl font-bold transition-colors flex items-center justify-center gap-3">
      ...
      <p className="text-center text-xs text-gray-600 mt-6 relative z-10">
        Note: This is a demo. In production, this would redirect to <code className="bg-[#0A0A0A] px-1 py-0.5 rounded">app.neonexus.io</code>
      </p>
    </div>
  );
}
...
export default function Login() {
  return (
    <div className="min-h-screen flex items-center justify-center bg-[#0A0A0A] py-20 px-6">
