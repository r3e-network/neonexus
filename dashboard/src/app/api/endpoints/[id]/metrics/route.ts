...
import { fetchRemoteNodeMetrics } from '@/services/nodes/RemoteNodeMetrics';
...
export async function GET(
  ...
  if (endpoint.type !== 'Dedicated') {
    return NextResponse.json(
      { error: 'Shared endpoint metrics are not directly available from the managed node UI.' },
      { status: 400 },
    );
  }
  ...
  if (!endpoint.providerPublicIp || !process.env.VM_OPERATOR_PRIVATE_KEY_PATH) {
    return NextResponse.json(
      { error: 'Remote node metrics access is not configured for this environment.' },
      { status: 400 },
    );
  }

  const metrics = await fetchRemoteNodeMetrics({
